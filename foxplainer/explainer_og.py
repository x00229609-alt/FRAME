...
class FoX(object):
    ...
    def __init__(self, global_model_name=None, xtype='abd', xnum=1, global_model_path=None, proj_name=None,
                 data_path=None, inst_id=0):
    ...
    def exp_to_html(self, exp_list=None, exp_type=None, explained_instance=None):
        for exp in exp_list:
            ...
            if self.explained_instance == "":
                self.explained_instance = HtmlString(list_of_pair=explained_instance, exp_type=self.options.xtype,
    ...
    def explain(self, in_jupyter=False):
        ...
        if options.xtype:
            ...
            for id in range(len(insts)):
                ...
                _, _, explained_instance, explanation_list, explanation_size_list, self.pred = self.explainer.explain(
                    inst)
                ...
                if in_jupyter:
                    ...
                    if self.options.xnum not in (-1, 'all'):
                        ...
                        # i.e. enumeration
                        # abd exp
                        self.exp_to_html(exp_list=explanation_list['abd'], exp_type='abd',
                                         explained_instance=explained_instance)
                        self.exp_to_html(exp_list=explanation_list['con'], exp_type='con',
                                         explained_instance=explained_instance)
