...
def build_tree(json_tree, node = None, feature_names = None, inverse = False):
    # Build name->index map once so we can accept XGBoost dumps that use
    # either numeric ids, "fN", or original feature names in "split".
    feature_to_idx = {}
    if feature_names is not None:
        for i, fname in enumerate(feature_names):
            s = str(fname)
            feature_to_idx[s] = i
            feature_to_idx[s.lower()] = i

    def to_feature_index(split_key):
        if isinstance(split_key, (int, np.integer)):
            return int(split_key)

        s = str(split_key).strip()
        if s.startswith('f') and s[1:].isdigit():
            return int(s[1:])
        if s.isdigit():
            return int(s)

        if s in feature_to_idx:
            return feature_to_idx[s]
        sl = s.lower()
        if sl in feature_to_idx:
            return feature_to_idx[sl]

        raise ValueError("Unsupported split feature key in XGBoost dump: {0}".format(split_key))
    ...
    def extract_data(json_node, root = None, feature_names = None):
        ...
        if "children" in json_node:
            ...
            node.missing_node_id = json_node.get("missing", json_node["no"])
            node.feature = to_feature_index(json_node["split"])
            if feature_names is not None and 0 <= node.feature < len(feature_names):
                node.name = feature_names[node.feature]
            node.threshold = json_node.get("split_condition", 0.0)
